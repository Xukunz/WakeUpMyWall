...
class MockDataTest {
    ...
    fun `metrics stay inside physically plausible bounds`() {
        ...
        listOf(metrics.cpuPercent, metrics.gpuPercent, metrics.ramPercent, metrics.storagePercent)
            .forEach { assertTrue(it in 0f..100f, "百分比越界：$it") }
        listOf(metrics.cpuTempC, metrics.gpuTempC, metrics.motherboardTempC, metrics.ssdTempC)
            .forEach { assertTrue(it in 0..120, "温度越界：$it") }
        listOf(metrics.cpuFanRpm, metrics.gpuFanRpm, metrics.caseFanRpm)
