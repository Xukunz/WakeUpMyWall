...
- [ ] **Step 2: 跑测试确认失败** → `dotnet test --filter MetricsSocketTests`
- [ ] **Step 3: 实现**（握手查 Token → `AcceptWebSocketAsync` → 循环 `SendAsync` 一帧 JSON → `Task.Delay(interval, ct)`；客户端断开或取消即退出循环）
- [ ] **Step 4: 跑全量测试确认通过**（`dotnet test WakeUpMyWall.Agent.slnx`）
- [ ] **Step 5: 提交** `feat: push metrics over a websocket`
...
- [ ] **Step 1: 写失败测试**（坏帧返回 null、好帧解析出字段、cadence 两条分支、ActivityClock 的边界：刚好 60 秒算 idle）
- [ ] **Step 2: 跑测试确认失败**
- [ ] **Step 3: 实现**
- [ ] **Step 4: 跑测试确认通过**
- [ ] **Step 5: 提交** `feat: add a websocket metrics stream and the act-idle cadence`
...
- [ ] **Step 1: 写失败测试**（注入假 stream：`flowOf(frame)` → Monitor 显示帧里的数字；`flow { throw }` → 回退到 HTTP 探针；Idle 时被丢掉的帧不进 UI）
- [ ] **Step 2: 跑测试确认失败**
- [ ] **Step 3: 实现**
- [ ] **Step 4: 跑测试 + `assembleDebug`**
- [ ] **Step 5: 提交** `feat: stream live metrics on the phone with an idle-aware cadence`

### Task C4: 端到端验收与文档

- [ ] **Step 1: 模拟器 + 本机 Agent 走一遍**（WS 帧进界面 → 拔掉 Agent 观察重连 → 恢复）
- [ ] **Step 2: 截图存 `docs/plans/screenshots/phase5c-*.png`，把实录写进本计划 §4**
- [ ] **Step 3: 更新 roadmap（Phase 5 全部完成）、README、version-matrix（ktor-client-websockets 行）并提交** `docs: record the phase 5c acceptance run`
