...
- **多磁盘 Storage**：Storage 卡片每块固定磁盘一页（`C:\ · 1/2`），页脚给一组翻页控件 `‹ ●─● ›`（左右按钮 + 参考图那种"点 + 当前页胶囊"的页数指示），横向拖动同样可翻页；Agent 用 `DriveInfo` 枚举全部固定盘，单块盘读失败只跳过那一块（`disks` 在载荷**顶层**，见 [agent-api.md](docs/plans/agent-api.md)）
